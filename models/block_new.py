...
class block:
    ...
    def __init__(self, node_property, w_uij, delta_t=1, i_mean=0.6, i_sigma=0.2, tau_i=10., h=0.5, g_T=0.06, V_h=-70,
        ...
        self.refractory_steps = (self.T_ref / self.delta_t).type(torch.uint8)
        self.t_ik_last = - torch.ones([N], device=self.w_uij.device, dtype=torch.int32) * self.refractory_steps  # shape [N]
        ...
        self.t = torch.tensor(0., device=self.w_uij.device, dtype=torch.int32)  # scalar
    ...
    def run(self, debug_mode, noise_rate=None):
        ...
        # mean_psc = []
        # mean_activate = []
        # for i in range(2, self.sub_idx.max().int() + 1):
